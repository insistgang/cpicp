...
> ⚠️ 本 src 按**官方赛题原文**(docs 内 23-…docx)构建。早期 README 写的"Qwen2.5-VL+DINOv3 篡改定位/真伪鉴别"是**读错赛题**,已废弃——真任务见下。
...
| 文件 | 作用 | 自测 |
|---|---|---|
| `prepare_data.py` | 数据清单(类型/相似组/客户/业务标签)加载 + 分类集/相似度对构建 + **合成数据生成器**(无真实数据也能跑通流水线) | `--selftest` |
| `metrics.py` | **阈值扫描 P/R/F1 + 最优阈值选取 + ROC-AUC + Top-k 检索准确率**(官方核心交付:阈值分析) | `python metrics.py` |
| `similarity.py` | 嵌入相似度索引 + **去重检测**(高相似对,标记跨客户套用 vs 同客户重复) | `python similarity.py` |
| (待建) `classify.py` | CLIP 零样本/微调 分类器筛面签照片(需 torch/open_clip,在算力机上跑) | — |
| (待建) `embed.py` | 度量学习嵌入训练/提取(ArcFace/对比) | — |
| (待建) `app_demo.py` | Gradio/FastAPI 检测汇总视图(统计+可疑对+业务数据) | — |

## runbook(拿到合成数据后)
```bash
pip install -r requirements.txt           # 见下(待补 torch/open_clip/faiss)
# 0) 先用合成数据验证流水线(无需真实数据)
...
# 2) 分类筛面签 → 3) 嵌入+相似度去重 → 4) 阈值分析 → 5) demo(见各脚本)
```
