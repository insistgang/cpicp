...
|---|---|---|---|---|---|---|
| 1 | **01 网安** | AI Agent 安全闭环 (第五届) | ~82% | 13/13 模块自测通过 | 攻防闭环 demo.html + ROC/PR图 (AUC 0.998) + 攻击轨迹JSON | 官方赛题/靶场/报名均未发布; 去年作品需找回; Chaspark 答疑 |
| 2 | **02 金融科技** | 多模态影像分类+相似度去重 (无锡农商行命题) | ~72% | 10/10 模块自测通过; 真实像素端到端 (AUC 0.99/Top-1 1.0) | 合成票据/证照影像 + 去重可视化热图 + manifest | 签 NDA 领数据 (苏老师); 报名截止 8/17 (最早红线); CLIP真特征需算力机 |
| 3 | **03 智能建造** | 低空水上救援视觉 (分类3类+边缘30FPS) | ~62% | 13/13 项自测通过; train/eval/export 骨架待 GPU | 技术方案 Word初稿(617KB,内嵌4图) + 性能报告3图 + 海面增广 before/after | 数据 (李老师); Jetson Orin 硬件; 报名截止 8/20 |
| 4 | **04 AI 创新** | AOI few-shot 异常检测 + 违建 AI+X 线 | ~85% | 11/11 模块自测通过; 真实合成图端到端 (AUC 0.99/F1 0.94) | 合成AOI工件图 + 缺陷定位热力图 + ROC/PR + 异常分布图 | 真数据 (DAGM2007/MVTec); GPU <200ms 红线; 华为 chaspark; 报名策略 |
| 5 | **05 未来飞行器** | 低空未来飞行器应用场景设计 (纯方案/报告书) | ~55% | 无 src 要求; 报告书初稿 7 章+附录 | 正式 Word 报告书(640KB,4图嵌入§2.1/§2.4/§4.2/§5) + 4张架构/流程/商业/矩阵图 | 队员招募; docx→PDF需Office机; 真实成本调研 |
| 6 | **06 文化中国** | 数字人文可视化 (赛题2: 古籍互文+传播) | ~55% | 5/5 模块自测通过 | **8页16:9 PPTX草稿(逐字稿写入备注) + 6张配图嵌入** + 静态HTML | 组队报名 (9/14 截止); 《颜氏家训》全文史料+书影; 录语音 |
...
- **第二轮新增 (方向C 答辩杀手锏 + 方向E benchmark)**: `attack_demo.py` (4条合成攻击轨迹→检测命中→自动生成防御策略→拦截器阻断的完整闭环, 实跑 4/4 闭环成功, 攻击得手步数 2→0, 拦截 4→8) / `build_demo_html.py` (单文件静态 `output/demo.html` 140KB, 内嵌数据+图, 双击即开, 已 headless Chrome 渲染核验) / `benchmark_plot.py` (检测内核在 400 条合成正负轨迹上 **AUC=0.998, 检出 1.0/误报 0.035/F1 0.983, 达官方双线**)。
...
## 02 金融科技大赛 — 10/10 模块自测通过 + 真实像素端到端
...
- 新增: `run_all_selftests.py` (**10 模块全通过, 零警告**) / `Dockerfile` / `pr_curve.png` / `.gitignore` (排除 121 张可重生合成图, 保留 dedup_viz.png+manifest)。
...
## 03 智能建造大赛 — 13/13 项自测通过 + 技术方案 Word 初稿
...
- **交付物**: `output/docx/技术方案_初稿.docx` (617KB OOXML, 内嵌4图) + 5张 PNG (海面增广×2 + 性能报告×3)。
- 新增 `run_all_selftests.py` (**13 项全通过**); 修复 PR 曲线反向 bug (改 COCO 风格 maximum.accumulate 精度包络) + np.trapz→trapezoid。
...
## 04 AI 创新大赛 — 11/11 模块自测通过 + 真实合成图端到端
...
- 新增 `run_all_selftests.sh` (**11 模块全通过**) + `.gitignore` (排除可重生 npz/批量合成图)。
...
- **交付物**: `output/项目报告书_低空违建巡查系统.docx` (640KB OOXML, 读回校验: 段落155/非空123/表格8/内嵌图4/标题36, 4图精确命中 §2.1/§2.4/§4.2/§5 锚点) + `output/figures/` 4 张 PNG。
...
## 06 文化中国大赛 — 5/5 模块自测通过 + 8页 PPTX 草稿(重头戏)
...
- **交付物**: `output/作品_初稿.pptx` (400KB OOXML, 读回校验: **页数=8, 16:9 画布, 每页备注非空 8/8, 含配图页 6, 讲解稿估时 125s≤300s, 匿名敏感词扫描通过**; 已用 LibreOffice 转 PDF 逐页肉眼核验中文/版式正常) + 6 张 PNG。
...
> **第二轮小结**: 本地代码与可生成交付物已全部做掉 (6 赛自测全绿: 13+10+13+11+5 模块 + 05 docx; 实际交付物草稿: 攻防 demo.html / 真实像素端到端指标 / 技术方案与报告书 Word / 8页 PPTX)。**剩余全部为上表中需用户/硬件/数据/外部素材的前置事项**——Mac 本地已推到边界。
