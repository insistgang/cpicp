...
| 3 | **03 智能建造** | 低空水上救援视觉 (分类3类+边缘30FPS) | ~65% | 14/14 项自测通过; train/eval/export 骨架待 GPU | 技术方案+性能报告 双 Word初稿(617+239KB,内嵌7图) + 海面增广 before/after | 数据 (李老师); Jetson Orin 硬件; 报名截止 8/20 |
...
- 新增 `run_all_selftests.py` (**14 项全通过**); 修复 PR 曲线反向 bug (改 COCO 风格 maximum.accumulate 精度包络) + np.trapz→trapezoid。
...
> **第二+三轮小结**: 本地代码与可生成交付物已全部做掉 (6 赛自测全绿: 01=13 / 02=10 / 03=14 / 04=11 / 06=5 模块 + 05=3 渲染模块; 实际交付物草稿: 攻防 demo.html(10场景闭环) / 真实像素端到端指标 / 03 技术方案+性能报告双Word / 05 正文+附件双Word / 06 8页 PPTX(讲稿280s))。**剩余全部为上表与下表中需用户/硬件/数据/外部素材的前置事项**——Mac 本地已推到边界。
