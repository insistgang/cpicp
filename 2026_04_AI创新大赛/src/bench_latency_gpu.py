...
两档切换:
  - **GPU 档**:GPU 机装好 torch/timm(见 setup_gpu.sh),加 `--real` → 走 TimmBackend(真特征),
    红线 <200ms@2060。__在 2060 级显卡上跑本脚本即出 GPU 档__。
  - **CPU 档**:默认 ClassicBackend,红线 <2s。本机离线即可实测出 CPU 档。

用法:
  python3 bench_latency_gpu.py                # 默认 2500×2500,CPU 经典后端,n=10 次
  python3 bench_latency_gpu.py --real         # GPU 真特征档,需 torch/timm + 已缓存权重
...
def bench(size=2500, runs=10, bank_imgs=8, seed=0, warmup=2, prefer_real=False):
    """返回 (backend, is_real, avg_stages, p95_total)。"""
    backend, is_real = get_backend(prefer_real=prefer_real, grid=8)
...
def _selftest():
    ...
    check(not is_real, "自测强制走 CPU 经典后端(离线可复现)")
...
def main():
    ...
    backend, is_real, avg, p95 = bench(size=a.size, runs=a.runs, bank_imgs=a.bank,
                                      seed=a.seed, prefer_real=a.real)
